...
def connect_database():
    try:
        ...
        return
    # A simple table that has the time
    query = 'SELECT * FROM time_sche'
    my_cursor.execute(query)
    rows = my_cursor.fetchall()
    for data in rows:
        my_tree.insert(parent='', index='end', text='', values=(data[0], data[1]))
    mydb.commit()
    mydb.close()
...
vsb.place(x=487, y=77, height=173)
my_tree.configure(yscrollcommand=vsb.set)

# Define our columns.
my_tree['columns'] = ('Day', 'Time')

# Format our columns.
my_tree.column('#0', width=0, stretch=NO)
my_tree.column('Day', anchor=W, width=200)
my_tree.column('Time', anchor=W, width=200)
